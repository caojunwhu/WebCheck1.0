...
namespace DLGCheckLib
{
     ...
     * TopologyChecker topocheck = new TopologyChecker(mainlogyDataSet);//传入要处理的要素数据集  
        topocheck.PUB_TopoBuild("testTopo2");//构建拓扑的名字  
     ...
     * */
    public class TopologyChecker//功能：构建拓扑，拓扑检测
    {
        ...
        private IFeatureClass OpenFeatureClass(int pID)
        {
            ...
            {
                rClass = ArcGISHelper.GetFeatureClass(this.FeatureWorkspace as IWorkspace, pID);
        ...
        //新建一个TopologyDatasetName，将LayersTobeCheck，添加进数据集，重命名为要素+“_”；
        public TopologyChecker(CheckItem paras,List<string>LayersTobeCheck, string TopologyDatasetName)
        {
            Topocheckrecordcollection = paras.topocheckrecordcollection;
            CheckParas = paras;
            IMap localMap = CheckParas.localMap;
            IFeatureDataset IN_MainlogyDataSet=null;
            IEnumDataset pEnumDataset = CheckParas.pWorkSpace.get_Datasets(ESRI.ArcGIS.Geodatabase.esriDatasetType.esriDTFeatureDataset);
            List<string> topolayers = new List<string>();
            IDataset pTOPODataset = null;
            ...
            while (pDataset != null)
            {
                ...
                if (pDataset.Name == TopologyDatasetName)
                {
                    //如果数据集中有了topo图层，则不能简单的将数据集转换为IFeatureClassContainer，需要进行数据清理
                    IFeatureClassContainer Temp_FeatureClassContainer =  pDataset as IFeatureClassContainer;
                    ...
                    {
                        if (LayersTobeCheck.IndexOf(Temp_FeatureClass.AliasName) >= 0)
                            topolayers.Add(Temp_FeatureClass.AliasName);

                        Temp_FeatureClass = Temp_EnumFeatureClass.Next();
                    }
                    pTOPODataset = pDataset;
                }
                pDataset = pEnumDataset.Next();
            }

            //创建TOPO Dataset
            if (pTOPODataset == null)
            {
                IFeatureWorkspace pFeatureWorkSpace = CheckParas.pWorkSpace as IFeatureWorkspace;
                FeatureWorkspace = pFeatureWorkSpace;
                pTOPODataset = pFeatureWorkSpace.CreateFeatureDataset(TopologyDatasetName, localMap.SpatialReference);
            }
            ...
            //string sTopoWorkspacepath = string.Format("{0}\\{1}", localCheckProject.DWGPath, "TOPO");
            //IWorkspace pTopoWorkspace = pWorkspaceFactory.OpenFromFile(sTopoWorkspacepath, 0);
            //IWorkspace pTopoWorkspace = pTOPODataset as IWorkspace;
            //向TOPO中添加图层
            var q = from r in LayersTobeCheck
                    where topolayers.IndexOf(r) < 0
                    select r;

            foreach (string layer in q)
            {
                string topoFeatureName = string.Format("{0}_{1}", layer, TopologyDatasetName);
                if (topolayers.IndexOf(topoFeatureName) < 0)
                {
                    if (pTOPODataset == null) break;
                    ...
                    IFeatureLayer ilayer = localMap.Layer[index] as IFeatureLayer;
                    string layername = ilayer.Name;
                    if (layername != layer) continue;

                    if (ilayer == null) continue;
                    IDataset pFeauteDataset = ilayer.FeatureClass as IDataset;
                    if (pFeauteDataset == null) continue;


                    ArcGISHelper.CreateFtCls(pFeauteDataset, pTOPODataset as IFeatureDataset, topoFeatureName);
                }
            }

            FeatureDataset_Main = IN_MainlogyDataSet = pTOPODataset as IFeatureDataset;
            if (LI_FeatureClass.Count != 0)
                LI_FeatureClass.Clear();
            PUB_GetAllFeatureClass();//获取数据集中所有包含的要素类
        }
        ...
        {
            ITopologyContainer topologyContainer =  FeatureDataset_Main as ITopologyContainer;
            try//若不存在同名拓扑则添加
            {
            ...
            if (Topology != null)
            {
                ...
                while (Temp_ErrorFeature != null)
                {
                    ...
                    if (Temp_ErrorFeature.DestinationClassID != 0)//检测是否是双要素规则
                    {
                        ...
                        //双要素规则添加拓扑错误
                        TopocheckRecord newRecord = new TopocheckRecord();
                        newRecord.ProjectID = Topocheckrecordcollection.ProjectID;
                        newRecord.MapNumber = Topocheckrecordcollection.MapNumber;
                        newRecord.SerialNumber = Convert.ToString(Topocheckrecordcollection.ResultList.Count + 1);
                        newRecord.Category = CheckParas.Category;
                        newRecord.CheckType = CheckParas.CheckType;
                        newRecord.CheckItemName = PRV_RecorverTopologyRuleType((int)(Temp_ErrorFeature.TopologyRuleType));
                        newRecord.FeatureClassA = GetFeatureClassName(Temp_ErrorFeature.OriginClassID).Replace("_"+CheckParas.ClassName,"");
                        newRecord.FeatureClassB = GetFeatureClassName(Temp_ErrorFeature.DestinationClassID).Replace("_" + CheckParas.ClassName, "");
                        newRecord.FeatureA = Temp_ErrorFeature.OriginOID.ToString();
                        newRecord.FeatureB = Temp_ErrorFeature.DestinationOID.ToString();
                        newRecord.Attribute = "";
                        newRecord.Remark = string.Format("图层{0}的要素{1}与图层{2}的要素{3}{4}检查失败；", newRecord.FeatureClassA, newRecord.FeatureA,newRecord.FeatureClassB,newRecord.FeatureB, newRecord.CheckItemName); ;
                        newRecord.CheckTime = DateTime.Now.ToLongDateString();
                        newRecord.Checker = "";

                        string geomWkt = Utils.Converters.ConvertGeometryToWKT(Temp_Feature.Shape);
                        string geomType = Temp_Feature.Shape.GeometryType.ToString();
                        newRecord.GeomWkt = geomWkt;
                        newRecord.GeomType = geomType;

                        Topocheckrecordcollection.ResultList.Add(newRecord);
                    }
                    else
                    {
                        ...
                        newRecord.FeatureA = Temp_ErrorFeature.OriginOID.ToString();
                        newRecord.Attribute = "";
                        newRecord.Remark = string.Format("图层{0}要素{1}{2}检查失败；",newRecord.FeatureClassA,newRecord.FeatureA, newRecord.CheckItemName);
                        newRecord.CheckTime = DateTime.Now.ToLongDateString();
                        newRecord.Checker = "";
        ...
        private esriTopologyRuleType PRV_ConvertTopologyRuleType(TopoErroType IN_TopoRuleType)
        {
            ...
            switch (IN_TopoRuleType)
            {
                ...
                case TopoErroType.点要素之间不相交:
                    ...
                    break;
                case TopoErroType.点要素重合点要素:
                    Temp_TopoRuleType = esriTopologyRuleType.esriTRTPointCoincidePoint;
                    break;
